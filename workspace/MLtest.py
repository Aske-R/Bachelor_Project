...
def compute_lgbm(alg,path_to_data,path_to_tree,path_to_weights,name_reqs,name_excepts,useTrainCV=True,small_size=True):
    ...
    train_var , sig_var = tree.pandas.df(leaf_names_p) , tree.pandas.df('Signal')
...
name_reqs= [b'p_']
name_excepts=[b'mc',b'weight',b'Signal',b'Truth',b'LHVal']
xgb1 = XGBClassifier(
 learning_rate =0.1,
 n_estimators=1000,
 max_depth=5,
 ...
 scale_pos_weight=1,
 seed=27)
model, X_train, X_test, y_test, y_pred, y_train= compute_xgboost(str(Path(__file__).resolve().parent.parent)+"/data/MixedTest.root",b'tree;1',str(Path(__file__).resolve().parent.parent)+'/data/total_weights_test',name_reqs,name_excepts)
